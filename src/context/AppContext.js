...
export const AppProvider = ({ children }) => {
  ...
  const getInventoryProducts = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(
        `https://hospital-ms-api.herokuapp.com/inventory/products/available/admin?page=0&size=20`,
        {
          headers: {
            "content-type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const getInventoryProductsUser = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(
        `https://hospital-ms-api.herokuapp.com/inventory/products/available/user?page=0&size=20`,
        {
          headers: {
            "content-type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
